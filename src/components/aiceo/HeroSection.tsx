...
export function HeroSection({ onRunClick }: HeroSectionProps) {
  return <section className="relative min-h-screen w-full overflow-hidden flex flex-col items-center justify-start">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-4 pt-10 sm:pt-16 pb-10 flex flex-col items-center text-center">
        {/* Headline */}
        <h1 className="leading-[1.05] tracking-tight mb-5 my-0 mx-0 text-[32px] sm:text-[clamp(36px,5.5vw,68px)]" style={{
        ...
        {/* Floating footer */}
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 10, background: "rgba(5,7,15,0.85)", backdropFilter: "blur(8px)", borderTop: "1px solid rgba(255,255,255,0.06)", padding: "10px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <span style={{ fontSize: 12, color: "rgba(255,255,255,0.3)" }}>© {new Date().getFullYear()} Timewarp · <a href="mailto:vincentackermann@timewarpdev.com" style={{ color: "rgba(255,255,255,0.4)", textDecoration: "none", transition: "color 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.6)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.4)"; }}>vincentackermann@timewarpdev.com</a></span>
          <div style={{ display: "flex", gap: 16 }}>
            <Link to="/pricing" style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", textDecoration: "none", transition: "color 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.6)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.3)"; }}>Pricing</Link>
            <Link to="/privacy" style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", textDecoration: "none", transition: "color 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.6)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.3)"; }}>Privacy Policy</Link>
            <Link to="/terms" style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", textDecoration: "none", transition: "color 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.6)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "rgba(255,255,255,0.3)"; }}>Terms & Information</Link>
          </div>
        </div>
      </div>
    </section>;
