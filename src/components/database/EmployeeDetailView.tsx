            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  ...
                  { id: "hubspot", name: "HubSpot", desc: "CRM, Contacts, Deals", logo: logoHubspot, comingSoon: true },
                ].map(integration => {
                  const connected = !!connectedProviders[integration.id];
                  return (
                    <div key={integration.id} className={`flex flex-col gap-2 p-4 rounded-xl border transition-all ${(integration as any).comingSoon ? "border-border/50 opacity-60" : connected ? "border-primary/40 bg-primary/5" : "border-border/50"}`}>
                      ...
                      {(integration as any).comingSoon ? (
                        <span className="text-[10px] font-medium px-2.5 py-1 rounded-full bg-muted text-muted-foreground text-center">Coming Soon</span>
                      ) : connected ? (
