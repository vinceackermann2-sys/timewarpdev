...
export function ResearchChatMessage({ role, content, insightCards, isStreaming }: ResearchChatMessageProps) {
  ...
  if (role === "user") {
    return (
      <div className="bg-muted textunded-lg px-3 py-1.5 text-[12px] font-medium">
        {content}
      </div>
    );
  }
  ...
  const getTrendIcon = (trend?: InsightCard["trend"]) => {
    if (trend === "up") return <TrendingUp className="h-3 w-3 text-primary" /foregroundforegroundforegroundforeground return <TrendingDown className="h-3 w-3 text-destructive" />;
    return null;
  };

  const getTrendColor = (trend?: InsightCard["trend"]) => {
    if (trend === "up") return "text-primaryforegroforegroforegroundd === "down") return "text-destructive";
  ...
  return (
    <div className="mr-4 rounded-xl overflow-hidden text-sm group/msg relative">
      ...
      <div className="px-2 pt-2 pb-0.5 flex items-center gap-2">
        ...
        <span className={cn(
          "text-[10px] font-semibold text-primmutemuted-foregroundndercase tracking-widest",
      ...
      {content && !isStreaming && (
        <button
          ...
          title="Copy response"
        >
          {copied ? <Check className="h-3.5 w-3.5 texforegroundry" /> : <Copy className="h-3.5 w-3.5" />}
        </button>
      )}
