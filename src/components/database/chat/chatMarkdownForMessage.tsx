...
export function buildChatMarkdownComponents(msg: ChatMessage): Partial<Components> {
  ...
  return {
    h1: ({ children }) => <h1 className="text-xl font-bold text-foreground mt-7 mb-4 tracking-tight first:mt-0">{children}</h1>,
    h2: ({ children }) => <h2 className="text-lg font-semibold text-foreground mt-7 mb-3 tracking-tight first:mt-0">{children}</h2>,
    h3: ({ children }) => <h3 className="text-base font-semibold text-foreground mt-6 mb-2 tracking-tight first:mt-0">{children}</h3>,
    p: ({ children }) => <p className="mb-4 last:mb-0 leading-[1.8] text-foreground/90">{children}</p>,
    ul: ({ children }) => <ul className="my-4 pl-6 space-y-2.5 list-disc marker:text-foreground/30">{children}</ul>,
    ol: ({ children }) => <ol className="my-4 pl-6 space-y-2.5 list-decimal marker:text-foreground/30">{children}</ol>,
    li: ({ children }) => <li className="leading-[1.8] text-foreground/90 pl-1">{children}</li>,
