...
function SourceChip({ card }: { card: DashboardCard }) {
  ...
  return (
    <div className="px-4 py-2.5 flex items-center gap-2 bg-[#ECEAF8] border-b border-border/40">
      {sourceMeta.icon ? (
        <img
          src={sourceMeta.icon}
          alt=""
...
export function DashCardDetailPanel({ card, open, onClose, onExecuteAction, minimized: minimizedProp, onMinimizedChange, onTrackEvent }: Props) {
  ...
  if (minimized) {
    return (
      <aside
        className="hidden md:flex w-[420px] shrink-0 self-end mb-12 mx-3 flex-col rounded-2xl border border-border overflow-hidden bg-[#F8F7FF] shadow-[0_0_10px_2px_hsl(210_20%_85%/0.55)] transition-all duration-300 ease-in-out animate-fade-in cursor-pointer"
      >
  ...
  return (
    <TooltipProvider delayDuration={250}>
      <aside className="hidden md:flex w-[420px] shrink-0 h-[calc(100%-6rem)] my-12 mx-3 flex-col rounded-2xl border border-border overflow-hidden bg-[#F8F7FF] shadow-[0_0_10px_2px_hsl(210_20%_85%/0.55)] transition-all duration-300 ease-in-out animate-fade-in">
        {/* ── Top bar ─────────────── */}
        ...
        <div className="shrink-0 px-6 pb-5 pt-2 bg-[#F8F7FF] shadow-xl">
          <Button
            size="sm"
