...
function WorkflowTab({ agent }: { agent: AIAgent; onUpdated: (a: AIAgent) => void }) {
  ...
  return (
    <div className="flex-1 relative min-h-0 overflow-hidden rounded-xl border border-border/60 bg-[radial-gradient(circle,_hsl(var(--border))_1px,_transparent_1px)] [background-size:16px_16px]">
      {/* Top-left badges floating on canvas */}
      <div className="absolute top-4 left-4 z-20 flex flex-col gap-2 pointer-events-none">
        <Badge variant="secondary" className="text-[11px] capitalize w-fit shadow-sm bg-background/80 backdrop-blur">
          Trigger: {agent.trigger_type}
        </Badge>
        <span className="text-[11px] text-muted-foreground/80 font-medium">
          Hover nodes for details. Edit in Settings.
        </span>
      </div>
