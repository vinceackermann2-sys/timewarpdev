...
function PlanCard({
  ...
  return (
    <div className={cn("relative rounded-2xl border border-border flex flex-col shadow-sm bg-white", "p-8")}>
      {/* Badge top right */}
      {badge && (
      ...
      <Button
        ...
        disabled={loading}
        className={cn(
          "w-full h-12 rounded-full font-semibold mb-8 text-base",
...
export default function PricingPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return (
    <div className={cn("bg-background", !embedded && "min-h-screen")}>
      ...
      <div className={cn("max-w-5xl mx-auto px-4", embedded ? "pb-4 space-y-6" : "pb-16 space-y-10")}>
        {/* Header */}
        <div className={cn("text-center", embedded ? "pt-0" : "pt-8")}>
          <h1 className={cn("font-bold tracking-tight", embedded ? "text-3xl" : "text-4xl sm:text-5xl")}>Plans & Pricing</h1>
        </div>
