...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'execute') {
      ...
      const sendCDP = async (method: string, params: Record<string, unknown> = {}, sessionId?: string): Promise<unknown> => {
      ...
      try {
        ...
        const targetsResult = await sendCDP('Target.getTargets') as { targetInfos: Array<{ targetId: string; type: string; url: string }> };
        ...
        // Send commands to the page session
        const sendPageCDP = (method: string, params: Record<string, unknown> = {}) => 
          sendCDP(method, params, cdpSessionId);
...
- "instructions": (only for login_required) step-by-step instructions for the user
...
- In "reasoning", explain what login is needed
- In "instructions", provide clear step-by-step instructions
...
- For searches, navigate to the site first, then click the search box and type
...
- Return ONLY valid JSON, no markdown.`;
        ...
        const getElementCenter = async (backendNodeId: number): Promise<{ x: number; y: number } | null> => {
          try {
        ...
        switch (agentAction.action) {
          ...
          case 'click':
            if (agentAction.target) {
              ...
              const coords = await getElementCenter(backendNodeId);
              if (coords) {
    ...
    throw new Error("Invalid action specified. Valid actions: create, execute");
