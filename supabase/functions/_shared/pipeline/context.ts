...
export async function buildAssistantPipelinePrompt(
  ...
  const toolDefinitionsBlock =
    ...
    "- **Slack bot reality check**: Creating a TimeWarp Agent named like a Slack bot does NOT install a new standalone Slack app/bot into the user's Slack workspace. The real Slack workspace install happens only when the user connects Slack through Connectors. After that, agents can post through the connected TimeWarp Slack app using `chat:write` and optional persona fields (`username`, `icon_url`, `icon_emoji`). Never claim you created/installed/invited a real Slack bot in Slack. If Slack is not connected, stop and tell the user to connect Slack first. If they want a true custom Slack App with inbound mentions/events, say that requires the custom Slack App + webhook route, not normal agent creation.\n" +
    "- **Reality check before calling `create_agent` (CRITICAL — do not skip)**: This platform currently supports `trigger_type: 'manual'` (user clicks Run) and `'schedule'` (polled on a cron). It does **NOT** yet have a live event bus — there is no webhook listener wired up for Gmail push notifications, Slack events, Discord messages, Stripe webhooks, calendar invites, or any other inbound event source. So:\n" +
    "  • If the user asks for something like 'notify me when a Gmail comes in', '@mention bot in Slack', 'alert me on a new Stripe charge', 'reply when someone posts' — DO NOT silently create an `event` agent and claim it works. It will sit in draft and never fire.\n" +
    "  • Be honest: explain the limitation in one sentence, then offer the working alternative — a `schedule` agent that polls (e.g. 'check Gmail inbox every 5 min and Slack-message me about new messages from VIPs'). Confirm the schedule + filter with the user, THEN call `create_agent` with `trigger_type:'schedule'`.\n" +
    "  • For Slack inbound (`@mentions`, channel Q&A): also offer the custom Slack App route (user creates the app at api.slack.com, we wire a webhook edge function).\n" +
    "  • Newly created agents land as `status:'draft'` and only run when the user enables them in Workforce → Agents. Always tell the user this last step.\n\n" +
