...
import { SKILLS_BUNDLE as SKILL_BUNDLE } from "./_skills_bundle.ts";
...
export function buildSkillsBlock(matched: Skill[]): string {
  ...
  const header =
    ...
    "For **vague growth / strategy** asks, deliver a concrete plan first; if clarification is needed, you may ask multiple batched questions in the same turn using multiple `[SUGGEST:…]` tags (the UI will handle them as a multi-step flow).\n\n";
