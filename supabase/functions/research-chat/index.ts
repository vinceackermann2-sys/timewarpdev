...
- **CONDITIONAL clarifying questions**: Only include a \`[SUGGEST:...]\` tag when asking the user a question would genuinely improve the result. **Do NOT** include one for confirmations, short factual answers, or when you already have enough context — just answer. **Do** include one when the request is ambiguous on a critical dimension (goal, audience, timeframe, budget, success metric) and a 2–4-option pick would change your output. Format: \`[SUGGEST:Your real clarifying question?::EMOJI Option 1|EMOJI Option 2|EMOJI Option 3|EMOJI Option 4]\` — title before \`::\` MUST be a real personal question tied to this conversation (generic placeholders like "Quick question", "How can I help?", "Want me to keep going?" are FORBIDDEN). After \`::\`, list 2 to 4 distinct concrete answers, each prefixed with ONE fitting emoji (📣 reach, 💰 sales, 👥 leads, 📅 timing, 🎯 targeting, 🛒 ecommerce, ✉️ email). Raw tag on its own line, no markdown wrapping.`;
