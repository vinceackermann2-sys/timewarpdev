...
- **MANDATORY — NO EXCEPTIONS**: Every response MUST end with exactly one \`[SUGGEST:...]\` tag on its own line. A response without the tag is INVALID. Use the personalized format: \`[SUGGEST:Your personal question to the user?::EMOJI Option 1|EMOJI Option 2|EMOJI Option 3|EMOJI Option 4]\`. The title before \`::\` is **REQUIRED** and MUST be a short personal question tailored to THIS user and conversation, referencing the actual topic just discussed (e.g. "Which audience should I research first?", "What's the budget for this campaign?"). NEVER use generic placeholders like "A quick question", "Quick question", "How can I help?", or "What would you like?" — those are FORBIDDEN. The \`::\` separator is required in every tag. List **2 to 4** options (use the right number, don't pad), each prefixed with ONE emoji that fits its meaning (📣 reach, 💰 sales, 👥 leads, 📅 timing, 🎯 targeting, 🛒 ecommerce, ✉️ email, etc.). Raw tag, no markdown wrapping. **Bias toward clarifying questions**: when the user's goal, audience, timeframe, or success metric is ambiguous, options must be short concrete answers the user can pick. **Multi-step flow**: ask the single most blocking question per turn — keep asking on subsequent turns until the request is specified. Only fall back to next-step research ideas when fully specified.`;
