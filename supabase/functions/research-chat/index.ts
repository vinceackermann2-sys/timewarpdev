...
serve(async (req) => {
  ...
  try {
    const { messages, dataSources, dataContext } = await req.json();
...
${dataContext ? `The user has connected the following data sources:\n${dataContext}\n` : ""}

Guidelines:
- Provide clear, concise answers based on the available data context
- If asked about data you don't have access to, explain what's available
- Suggest relevant follow-up questions to help users explore their data
- Use bullet points and structured formatting for clarity
- When analyzing patterns, be specific about what you observe`;
