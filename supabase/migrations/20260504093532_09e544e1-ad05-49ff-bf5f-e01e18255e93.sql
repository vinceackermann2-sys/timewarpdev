CREATE OR REPLACE FUNCTION public.increment_workspace_actions(_workspace_id uuid, _cost_usd numeric DEFAULT 0.08)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  total_cap := action_limit + current_bonus;
  remaining := total_cap - current_actions;
  ...
  new_used := LEAST(current_actions + consumed, total_cap);

  UPDATE public.workspace_subscriptions
    SET actions_used = new_used,
        updated_at = now()
    WHERE workspace_id = _workspace_id;

  RETURN json_build_object(
    'allowed', true,
    'actions_used', new_used,
    'consumed', new_used - current_actions,
    'cost_usd', COALESCE(_cost_usd, 0.08)
  );
END;
$function$;

CREATE OR REPLACE FUNCTION public.increment_actions_used(_user_id uuid, _cost_usd numeric DEFAULT 0.08)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
