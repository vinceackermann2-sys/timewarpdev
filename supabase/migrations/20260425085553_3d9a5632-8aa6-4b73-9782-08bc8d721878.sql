...
BEGIN
  ...
  IF NOT has_active_sub THEN
    -- Free tier: 100 actions per month
    action_limit := 100;
  ...
  -- Free tier and paid plans both consume from action_limit first, then from
  -- bonus_actions (purchased action packs). Bonus actions are uncapped — users
  -- can purchase as many as they want.
  IF current_actions >= (action_limit + current_bonus) THEN
    RETURN json_build_object('allowed', false, 'reason', 'Action limit reached. Purchase more actions to continue.');
  END IF;
